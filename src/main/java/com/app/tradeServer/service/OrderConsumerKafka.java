    ...
    public void consume(byte[] data) {
        ...
        if (stockExchangeTradeResponse.isStatus()) {
            if (userOrder.getOrderType().equals("sell")) {
                ...
                userFunds.setAmount(userFunds.getAmount() + (userStocks.getQuantity() * userOrder.getBuyPrice()));
            } else {
                // Handle buy order: Update UserStocks, UserFunds, and UserOrders
                userFunds.setHoldAmount(userFunds.getHoldAmount() - (userOrder.getQuantity() * userOrder.getBuyPrice()));
                userStocks.setQuantity(userOrder.getQuantity());
                userStocks.setBuyPrice(userOrder.getBuyPrice());
